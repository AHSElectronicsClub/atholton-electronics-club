...
def fetch_sensor_data(conn: psycopg2.extensions.connection, buoy_id: str, 
    ...
    try:
        if start_time and end_time:
            query = """
                SELECT "timestamp", pH, "DO", EC, Turbidity, Temp, ORP, rain_flag, water_leak, session_id, sample_count
                FROM sensor_data 
...
def get_dashboard_data(buoy_id: str, timeframe_start: str, timeframe_end: str, 
    ...
    try:
        ...
        # Calculate standard deviation excluding time_since_rain_min if it exists
        std_dev_columns = [col for col in df_processed_timeframe.columns if col != 'time_since_rain_min']
        std_dev = df_processed_timeframe[std_dev_columns].std(numeric_only=True).to_dict()
        
        moving_avg = df_processed_timeframe.rolling(window=12, min_periods=1, on='timestamp') \
                                           .mean(numeric_only=True)
        # Remove time_since_rain_min from moving average if it exists
        if 'time_since_rain_min' in moving_avg.columns:
            moving_avg = moving_avg.drop(columns=['time_since_rain_min'])
        ...
        latest_water_leak = bool(df_raw_timeframe.iloc[-1]['water_leak']) if 'water_leak' in df_raw_timeframe.columns else False
