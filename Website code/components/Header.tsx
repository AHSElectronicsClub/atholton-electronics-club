    ...
    <nav className="bg-navy sticky top-0 z-40 shadow-lg border-b border-teal/20">
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden md:block">
            <div className="ml-10 flex items-baseline space-x-4">
              {nav.map(item => (
                <a key={item.href} href={item.href} className="px-3 py-2 rounded-md text-base font-medium text-gray-300 hover:bg-navy-light hover:text-white">{item.name}</a>
              ))}
          ...
          <div className="md:hidden flex items-center">
            ...
            <button onClick={() => setIsMobileOpen(!isMobileOpen)} className="inline-flex items-center justify-center p-2 rounded-md text-gray-400 hover:text-white hover:bg-navy-light">
      ...
      <div className={`${isMobileOpen ? 'block' : 'hidden'} md:hidden border-t border-navy-light`}>
        <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
          {nav.map(item => (
            <a key={item.href} href={item.href} className="block px-3 py-2 rounded-md text-base font-medium text-gray-300 hover:bg-navy-light hover:text-white">{item.name}</a>
          ))}
        </div>
      </div>
    </nav>
