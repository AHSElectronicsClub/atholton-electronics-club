...
export default function ResourcesSection() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <h2 className="text-3xl font-bold font-display text-forest mb-8">Resources</h2>

      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mb-12">
        {extendedResources.map(resource => (
          <div
            key={resource.id}
            className="bg-off-white rounded-xl shadow-lg p-6 flex flex-col justify-between hover:shadow-2xl transition-shadow"
          >
            <h3 className="text-xl font-semibold text-forest mb-2">{resource.title}</h3>
            <p className="text-gray-600 text-sm flex-grow">{resource.description}</p>
            <a
              href={resource.link}
              target="_blank"
              rel="noopener noreferrer"
              className="mt-4 inline-block text-gold font-bold hover:underline"
            >
              Visit
            </a>
          </div>
      ...
      <div className="mb-12">
        <h3 className="text-2xl font-bold font-display text-forest mb-4">Example Code</h3>
